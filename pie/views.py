...
class ListPieView(ListView):
    ...
    def get_queryset(self):
        """
        Obtiene el conjunto de registros de PIE, filtrando por área académica, año y RUT si se proporcionan.
        ...
        rut_filtro = self.request.GET.get('rut_filtro')
        ...
        if rut_filtro:
            queryset = queryset.filter(estudiante__rut__icontains=rut_filtro)
    ...
    def get_context_data(self, **kwargs):
        ...
        # Filtrar los registros de PIE según el área académica, año y RUT seleccionados
        pies_list = RegistroPie.objects.all()
        if area_filtro:
            pies_list = pies_list.filter(estudiante__cursos__area_academica_id=area_filtro)
        if año_filtro:
            pies_list = pies_list.filter(año_registro=año_filtro)
        if rut_filtro:
            pies_list = pies_list.filter(estudiante__rut__icontains=rut_filtro)
